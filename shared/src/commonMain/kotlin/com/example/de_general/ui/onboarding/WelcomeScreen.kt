...
import com.example.de_general.ui.icons.ArrowForward
import com.example.de_general.ui.icons.ChevronRight
...
import com.example.de_general.ui.icons.WifiOff
import com.example.de_general.ui.onboarding.components.Badge
import com.example.de_general.ui.onboarding.components.DiagnosticRow
import com.example.de_general.ui.onboarding.components.DiagnosticRowSkeleton
...
private fun VerdictCard(report: CompatibilityReport?, probing: Boolean) {
    ...
    SectionCard {
        Row(
            ...
            ProgressRing(
                fraction = if (report == null) 0f else report.passed.toFloat() / report.total,
                primaryLabel = if (report == null) "—" else "${report.passed}/${report.total}",
                secondaryLabel = if (probing) "Checking" else "Checks passed",
                diameter = 116.dp,
                thickness = 10.dp,
                color = when (report?.verdict) {
                    CheckStatus.Fail -> MaterialTheme.colorScheme.error
                    CheckStatus.Warn -> MindfulTheme.moods.happy.accent
