...
import com.adll.de_general.feature.journal.domain.formatDayLabel
import com.adll.de_general.feature.journal.domain.formatShortDay
import com.adll.de_general.feature.journal.domain.formatTime
import com.adll.de_general.feature.journal.domain.journalMoodOrNull
import com.adll.de_general.feature.journal.ui.components.JournalDatePickerDialog
import com.adll.de_general.feature.journal.ui.components.PresetChip
 ...
 * [com.adll.de_general.navigation.CreateJournal], a full screen of its own.
 *
...
fun JournalScreen(
    ...
    onDismissError: () -> Unit,
    onFilterChange: (DateAnchor?) -> Unit,
    ...
    Surface(modifier.fillMaxSize(), color = MaterialTheme.colorScheme.surface) {
        Column(
            ...
            // Only once there is something to narrow down. An empty journal gets its own message
            // and no controls that would filter nothing.
            ...
            when {
                ...
                state.isFilteredEmpty -> Column(
                    verticalArrangement = Arrangement.spacedBy(spacing.sm),
                ) {
                    Text(
                        state.filteredEmptyMessage.orEmpty(),
                        style = MaterialTheme.typography.bodyLarge,
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                    )
                    TextButton(onClick = { onFilterChange(null) }) {
    ...
    if (state.pickingDate) {
        JournalDatePickerDialog(
            initialDate = state.pickerDate,
            nowMillis = state.nowMillis,
            zone = state.zone,
            confirmLabel = "Show this day",
...
/**
 * All · Today · Yesterday · Pick a date — the composer's date pills plus a way back to everything.
 *
 * Once a date is picked, the last pill names it, so the active filter is readable without opening
 * anything; tapping it again reopens the picker on that day. Scrolls sideways rather than wrapping,
 * so a narrow phone or large font keeps the row one line tall.
 */
@Composable
private fun DateFilterRow(
    ...
    Row(
        ...
        PresetChip("All", selected = filter == null, onClick = { onFilterChange(null) })
        PresetChip(
            "Today",
            selected = filter == DateAnchor.Today,
            onClick = { onFilterChange(DateAnchor.Today) },
        )
        PresetChip(
            "Yesterday",
            selected = filter == DateAnchor.Yesterday,
            onClick = { onFilterChange(DateAnchor.Yesterday) },
        )
        PresetChip(
            if (filter is DateAnchor.On) formatShortDay(filter.date) else "Pick a date",
            selected = filter is DateAnchor.On,
            onClick = onPickDate,
        )
    }
}
...
private fun EntryCard(
    ...
    SectionCard {
        ...
        Row(
            ...
            // A text button. MindfulIcons now has a trash glyph (Delete, added for the chat's
            // clear button); switching this card to it is a design choice left for later.
