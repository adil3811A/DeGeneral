...
/** "Oct 22, 2026". The picked-date filter pill, where the weekday would crowd the row. */
private val ShortDayFormat = LocalDate.Format {
    monthName(MonthNames.ENGLISH_ABBREVIATED)
    char(' ')
    day()
    chars(", ")
...
/**
 * The archive's date filter: the entries written on [filter]'s day in [zone], or all of them when
 * [filter] is null.
 *
 * Reuses [DateAnchor] rather than a filter type of its own — Today, Yesterday and a picked date
 * mean exactly what they mean in the composer, and null is "All". Days are calendar days in
 * [zone], the same way each row's "Today / Yesterday" label is decided, so a row labelled
 * "Yesterday" is always one the Yesterday filter keeps. Order is preserved: newest first in,
 * newest first out.
 */
fun entriesOn(
    ...
    val day = anchorDate(filter, nowMillis, zone)
    return entries.filter { localDateAt(it.timestamp, zone) == day }
}
